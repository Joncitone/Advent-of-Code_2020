...
function validByCount(policy) {
  const policyBySpace = policy.split(' ');
  const characterBlock = policyBySpace[1];
  const limits = policyBySpace[0].split('-');

  const lowerLimit = Number(limits[0]);
  const upperLimit = Number(limits[1]);

  const character = characterBlock[0];

  const password = policyBySpace[2];
  ...
  if (
    charCount[character] >= lowerLimit &&
    charCount[character] <= upperLimit
  ) {
    return true;
  } else {
    return false;
...
function validByPosition(policy) {
  const policyBySpace = policy.split(' ');
  const characterBlock = policyBySpace[1];
  const limits = policyBySpace[0].split('-');
  ...
  if (password[lowerLimit - 1] === character) charCount++;
  if (password[upperLimit - 1] === character) charCount++;

  return charCount === 1 ? true : false;
}

function validPasswordCounter(policies, callback) {
  let validPasswordTotal = 0;

  for (let policy of policies) {
    if (callback(policy)) validPasswordTotal++;
  }

  return validPasswordTotal;
}

console.log(validPasswordCounter(policies, validByPosition));
