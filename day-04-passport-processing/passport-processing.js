...
// Count the valid passports from the input with valid passports needing
...
for (i = 0; i < passports.length; i++) {
  let passportAsArray = passports[i]
    .split('\n')
    .flatMap((curr) => curr.split(' '));
  let passportObject = {};

  for (field of passportAsArray) {
    let splitField = field.split(':');

    let key = splitField[0];
    let value = splitField[1];

    passportObject[key] = value;
  }
  passports[i] = passportObject;
}

//now that passports is in a format that makes sense let's begin

function validPassportCounter(passports, requiredFields) {
  let validCount = 0;

  for (passport of passports) {
    let fieldsCount = 0;
    for (field of requiredFields) {
      if (passport[field]) fieldsCount++;
    }
...
console.log('Part One:', validPassportCounter(passports, requiredFields));
      ...
      switch (field) {
        ...
        case 'hgt':
          ...
          if (value.includes('in')) {
            let height = Number(value.split('in')[0]);
        ...
        case 'hcl':
          ...
          if (value[0] === '#' && value.length === 7) {
            for (let i = 1; i < value.length; i++) {
              if (validChars.includes(value[i])) validCharCount++;
            }
          }
          if (validCharCount === 6) {
            fieldsCount++;
...
console.log(
  'Part Two:',
  presentAndValidPassportCounter(passports, requiredFields)
);
