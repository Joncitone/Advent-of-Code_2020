...
let groupLines = groups.map((group) => group.split('\n'));

function countYesTotals(groupLines) {
  let groupTotals = [];

  //push count from letterCounter helperFunction
  for (group of groupLines) {
    groupTotals.push(letterCounter(group));
  }

  //return total by reducing groupTotals array
  return groupTotals.reduce((acc, curr) => {
    return acc + curr;
  }, 0);
}

function letterCounter(group) {
  let letterSet = new Set();

  //for each response, look at each letter and add it to the Set
  //Set works nicely for this since there won't be duplicate entries
  for (response of group) {
    for (answer of response) {
      letterSet.add(answer);
    }
  }
...
//Display the answer for part one
console.log('Part One:', countYesTotals(groupLines));

//STARTING PART TWO :)
//the function below is the same, except for the helper function call in the first
// for-loop, possible to make this more dynamic with a switch case based on part one/two
function countConsensusTotals(groupLines) {
  let groupTotals = [];

  for (group of groupLines) {
    groupTotals.push(consensusCounter(group));
  }
...
//the consensusCounter helper function works a little differently in that it looks for the
//intersection of all responses in a group, whereas the part one looked for the union
function consensusCounter(group) {
  let consensusSet = new Set();
  let groupSets = [];

  for (response of group) {
    let responseSet = new Set();
    for (answer of response) {
      responseSet.add(answer);
      groupSets.push(responseSet);
    }
  }

  let intersectionSet = groupSets[0];
  //now perform intersection operation on each set
  for (let i = 1; i < groupSets.length; i++) {
    let setB = groupSets[i];
    intersectionSet = intersection(intersectionSet, setB);
  }

  //intersection helper function taken from MDN Set documentation page
...
console.log('Part Two:', countConsensusTotals(groupLines));
